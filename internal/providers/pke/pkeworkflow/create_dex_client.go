...
func (a *CreateDexClientActivity) Execute(ctx context.Context, input CreateDexClientActivityInput) error {
	...
	err = a.clusterAuthService.RegisterCluster(ctx, cluster.GetOrganizationId(), cluster.GetUID())
	if err != nil {
		return errors.New(fmt.Sprintf("can't create dex client for cluster %t", cluster))
	}
